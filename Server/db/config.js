...
const { meetingFields } = require('./meetingFields.js');
    ...
    const createDynamicSchemas = async (CustomFields) => {
        for (const module of CustomFields) {
            ...
            if (!mongoose.models[moduleName]) {
                ...
                const schemaFields = {};
                function mapBackendTypeToMongooseType(backendType) {
                    if (backendType === 'String') return String;
                    if (backendType === 'Date') return Date;
                    if (backendType === 'Boolean') return Boolean;
                    if (backendType === 'ObjectId') return mongoose.Schema.Types.ObjectId;
                    if (backendType === 'ObjectId[]') return [{ type: mongoose.Schema.Types.ObjectId }];
                    if (backendType === 'String[]') return [String];
                    // Add more mappings as needed
                    return String; // fallback
                }
                for (const field of fields) {
                    schemaFields[field.name] = { type: mapBackendTypeToMongooseType(field.backendType) };
                }
...
const connectDB = async (DATABASE_URL, DATABASE) => {
    try {
        ...
        await createNewModule({ body: { moduleName: 'Meeting', fields: meetingFields, headings: [], isDefault: true } }, mockRes);
